...
void vfo_apply(void) {
    ...
    // Track the current running hardware chip mode across function calls
    static radio_mode_t last_hw_mode = MODE_COUNT; // Initialize to an invalid state

    if (vfo.mode == MODE_FM) {
        si5351_set_freq(0, 0, 0);   /* CLK0 off during FM receive */
        uint32_t fm_freq_10khz = rx / 10000;  
        if (fm_freq_10khz < 6800) fm_freq_10khz = 10500;	
		
        // Only trigger a heavy hardware switch if we weren't already in FM mode
        if (last_hw_mode != MODE_FM) {
            si4732_switch_mode(MODE_FM);
            sleep_ms(250); // Reduced delay to keep interface responsive
            last_hw_mode = MODE_FM;
        }
        
        si4732_tune_fm(fm_freq_10khz);
    } 
    else {
        // We are going to SSB or standard AM
        si5351_set_freq(0, (uint64_t)vfo.freq_hz, 4);  /* TX VFO */
        uint32_t freq_khz = rx / 1000;

        // 1. Determine the exact structural mode the Si4732 hardware needs to be in
        radio_mode_t target_hw_mode = is_ssb ? ((vfo.mode == MODE_LSB) ? MODE_LSB : MODE_USB) : MODE_AM;

        // 2. CRITICAL FIX: ONLY switch mode if the hardware mode has actually changed!
        if (last_hw_mode != target_hw_mode) {
            si4732_switch_mode(target_hw_mode);
            sleep_ms(250); 
            
            // Re-verify if the SSB patch successfully reloaded
            si4732_ssb_patched = si4732_is_ssb_loaded();
            last_hw_mode = target_hw_mode;
        }

        // 3. Perform standard tuning without touching the patch/mode registers
        si4732_tune_am(freq_khz);
...
/* ---- Deferred apply --------------------------------------------------- */
static volatile bool vfo_apply_flag = false;

void vfo_request_apply(void) { vfo_apply_flag = true; }

bool vfo_apply_pending(void) {
    if (!vfo_apply_flag) return false;
    vfo_apply_flag = false;
    vfo_apply();
    return true;
}
