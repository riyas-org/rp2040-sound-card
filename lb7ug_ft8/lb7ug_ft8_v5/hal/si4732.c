 ...
 * Based on working code from main.c
 */
#include <stdint.h>
#include <stdbool.h>
#include <string.h>
...
/* ── SSB patch ────────────────────────────────────────────────────────────
   Source: github.com/pu2clr/SI4735 (MIT licence, PU2CLR Ricardo Caratti)
   File  : extras/PATCHES/SI4735_PATCH_FULL_SSB.h
   Protocol: each 8-byte row = one I2C write after POWER_UP with PATCH=1.
   CTS polled between rows.  All-zero row = sentinel.
   To use the genuine patch, replace these rows with the real hex values. */
static const uint8_t ssb_patch[]={0x15, 0x00, 0x03, 0x74, 0x0B, 0xD4, 0x84, 0x60,
...
/* ========================================================================
   WORKING I2C FUNCTIONS – copied from your good main.c
   ======================================================================== */

static bool si4732_cts_wait(uint32_t cts_ms) {
    uint32_t t0 = to_ms_since_boot(get_absolute_time());
    for (;;) {
        uint8_t st = 0;
        i2c_read_blocking(I2C_PORT, SI4732_ADDR, &st, 1, false);
        if (st & 0x80) return true;
        if (st & 0x40) { printf("[SI4732] Error status: 0x%02X\n", st); return false; }
        if (to_ms_since_boot(get_absolute_time()) - t0 > cts_ms) {
...
static bool si4732_cmd(const uint8_t *c, int len, uint32_t cts_ms) {
    /* If called with NULL/0 it is a pure CTS poll, e.g. between patch rows. */
    if (c != NULL && len > 0) {
        int ret = i2c_write_blocking(I2C_PORT, SI4732_ADDR, c, len, false);
        if (ret < 0) {
            printf("[SI4732] I2C write error: %d\n", ret);
            return false;
        }
        sleep_ms(1);
    }
    return si4732_cts_wait(cts_ms);
}

static bool si4732_set_prop(uint16_t prop, uint16_t val) {
    uint8_t c[6] = {CMD_SET_PROPERTY, 0x00,
                    (prop>>8)&0xFF, prop&0xFF,
                    (val>>8)&0xFF, val&0xFF};
    return si4732_cmd(c, 6, 200);
}
...
void si4732_init_am(void) {
    reset_chip();
    uint8_t c[3] = {CMD_POWER_UP, 0x51, 0x05};  // AM mode, analog audio
    if (si4732_cmd(c, 3, 700)) {
        printf("[SI4732] AM power up OK\n");
    } else {
        printf("[SI4732] AM power up FAILED\n");
    }
    si4732_set_prop(PROP_AM_CHANNEL_FILTER, 3);
    si4732_set_prop(PROP_RX_VOLUME, 50);
    si4732_set_prop(PROP_RX_HARD_MUTE, 0);
}

void si4732_init_fm(void) {
    reset_chip();
    uint8_t c[3] = {CMD_POWER_UP, 0x50, 0x05};  // FM mode, analog audio
    si4732_cmd(c, 3, 700);
    si4732_set_prop(0x1101, 0x0001);  // FM channel spacing 100 kHz
    si4732_set_prop(0x1100, 0x0002);  // 50µs de-emphasis
    si4732_set_prop(PROP_RX_VOLUME, 50);
    si4732_set_prop(PROP_RX_HARD_MUTE, 0);
}
...
void si4732_apply_ssb_patch(void) {
    ...
    printf("[SI4732] Loading SSB patch (%u bytes)...\n", (unsigned)SSB_PATCH_LEN);
    
    // Power up with patch enable
    uint8_t pu[3] = {CMD_POWER_UP, 0x71, 0x05};  // PATCH=1, FUNC=AM
    if (!si4732_cmd(pu, 3, 700)) {
        printf("[SI4732] SSB power up failed\n");
        return;
    }
    ...
    while (rem >= 8) {
        if (i2c_write_blocking(I2C_PORT, SI4732_ADDR, p, 8, false) < 0) {
            printf("[SI4732] Patch row %d I2C error\n", row);
            return;
        }
        sleep_us(500);
        /* Poll CTS directly — do NOT call si4732_cmd(NULL,0,...) which
           would attempt a zero-length I2C write, corrupting the patch load. */
        if (!si4732_cts_wait(300)) {
            printf("[SI4732] Patch row %d CTS timeout\n", row);
            return;
        }
        p += 8;
        rem -= 8;
    ...
    if (rem > 0) {
        i2c_write_blocking(I2C_PORT, SI4732_ADDR, p, rem, false);
        sleep_ms(1);
        si4732_cts_wait(300);
    }

    si4732_ssb_patched = true;
    printf("[SI4732] SSB patch loaded (%d rows)\n", row);

    /* After patch, the chip is in patch-load state.
       Re-power-up in AM mode with SSB extensions enabled (FUNC=1, PATCH=0). */
    uint8_t pu2[3] = {CMD_POWER_UP, 0x51, 0x05};
    si4732_cmd(pu2, 3, 700);
    sleep_ms(10);

    /* Configure SSB properties */
    si4732_set_prop(PROP_SSB_MODE, 1);
    si4732_set_prop(PROP_SSB_BFO, 0);
    si4732_set_prop(PROP_AM_CHANNEL_FILTER, 4);
    si4732_set_prop(PROP_AM_SOFT_MUTE_MAX, 0);
    si4732_set_prop(PROP_RX_VOLUME, 50);
}

void si4732_tune_am(uint32_t freq_hz) {
    uint16_t khz = freq_hz / 1000;
    uint8_t c[5] = {CMD_AM_TUNE, 0x00, (khz>>8)&0xFF, khz&0xFF, 0x00};
    si4732_cmd(c, 5, 400);
    printf("[SI4732] Tuned to %u kHz\n", khz);
}

void si4732_tune_fm(uint32_t freq_10khz) {
    uint8_t c[5] = {CMD_FM_TUNE, 0x00, (freq_10khz>>8)&0xFF, freq_10khz&0xFF, 0x00};
    si4732_cmd(c, 5, 400);
}

void si4732_set_ssb_mode(radio_mode_t mode, uint8_t bw_code) {
    if (!si4732_ssb_patched) return;
    si4732_set_prop(PROP_SSB_MODE, (mode == MODE_LSB) ? 2 : 1);
    si4732_set_prop(PROP_SSB_BW, bw_code);
    si4732_set_prop(PROP_SSB_AGC, 0);
    si4732_set_prop(PROP_SSB_AFC, 0);
    si4732_set_prop(PROP_AM_SOFT_MUTE_MAX, 0);
    si4732_set_prop(PROP_AM_SOFT_MUTE_SNR, 8);
}

void si4732_set_bfo(int16_t hz) {
    if (hz > 16380) hz = 16380;
    if (hz < -16380) hz = -16380;
    si4732_set_prop(PROP_SSB_BFO, (uint16_t)hz);
}

void si4732_set_am_bw(uint8_t bw) {
    si4732_set_prop(PROP_AM_CHANNEL_FILTER, bw);
}

void si4732_agc_enable(bool on) {
    si4732_set_prop(PROP_AM_AGC_MAX_GAIN, on ? 0x01 : 0x00);
}
...
int8_t si4732_get_rssi(void) {
    uint8_t c[2] = {CMD_AM_STATUS, 0x01};
    si4732_cmd(c, 2, 100);
    uint8_t r[8] = {0};
    i2c_read_blocking(I2C_PORT, SI4732_ADDR, r, 8, false);
    return (int8_t)r[4];
}

void si4732_set_volume(uint8_t vol) {
    if (vol > 63) vol = 63;
    si4732_set_prop(PROP_RX_VOLUME, vol);
}

void si4732_mute(bool mute) {
    si4732_set_prop(PROP_RX_HARD_MUTE, mute ? 1 : 0);
}

void si4732_power_down(void) {
    uint8_t c = CMD_POWER_DOWN;
    i2c_write_blocking(I2C_PORT, SI4732_ADDR, &c, 1, false);
    sleep_ms(50);
}
